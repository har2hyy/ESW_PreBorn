#!/usr/bin/env python3
"""
Integrated YOLOv11 + Depth Anything V2 Pipeline (TFLite INT8 Version)
======================================================================

This pipeline uses the YOLOv11 INT8 TFLite model for optimized inference.
INT8 quantization provides:
- Faster inference on edge devices
- Smaller model size (4x reduction)
- Lower memory footprint
- Suitable for deployment on mobile/embedded systems
...
class IntegratedPipelineTFLiteINT8:
    """
    Integrated pipeline using TFLite YOLO model + Depth Anything V2.
    
    This version uses the TFLite YOLO model (FLOAT32 recommended).
    NOTE: The INT8 model has quantization issues where class predictions are stuck at zero-point.
    
    The pipeline uses:
    - OptimizedYOLOv11TFLite from tflite_yolo.py (tested and working)
    ...
    def __init__(self, 
        ...
        """
        Initialize the integrated pipeline with TFLite YOLO.
        
        Args:
            yolo_model_path: Path to YOLOv11 TFLite model (float32 recommended)
        ...
        """
        print("=" * 80)
        print("INITIALIZING INTEGRATED PIPELINE (TFLite Version)")
        print("Note: Using FLOAT32 model (INT8 has quantization issues)")
        print("=" * 80)
        
        # Store depth scale factor
        self.depth_scale_factor = depth_scale_factor
        
        # Initialize YOLO model using the tested OptimizedYOLOv11TFLite class
        print("\n[1/2] Loading YOLOv11 TFLite Model...")
        ...
        print(f"✓ YOLOv11 TFLite loaded with {len(self.yolo.class_names)} classes")
        ...
        print(f"Model Type: INT8 TFLite (Quantized)")
    ...
    def process_image(self, image_path: str, output_dir: str = None) -> Dict:
        ...
        print("[Stage 1/5] Running YOLO Object Detection (TFLite)...")
        ...
        print(f"✓ Depth map generated in {depth_time*1000:.1f}ms (range: {depth_map.min()}-{depth_map.max()})")
        ...
        for i, (box, conf, class_id) in enumerate(zip(boxes, confidences, class_ids)):
            ...
            print(f"  Object {i+1}: {class_name} - Avg Depth: {avg_depth:.1f}, Center: ({center_x}, {center_y})")
        ...
        if distances:
            ...
            for dist in distances[:5]:  # Show first 5
                print(f"  {dist['obj1_class']} ↔ {dist['obj2_class']}: "
                      ...
                      f"Depth diff: {dist['depth_diff']:.1f}")
        ...
        yolo_output_path = os.path.join(output_dir, f"{base_name}_yolo_detections_int8.jpg")
        cv2.imwrite(yolo_output_path, yolo_result)
        
        # Save depth map
        depth_output_path = os.path.join(output_dir, f"{base_name}_depth_map_int8.png")
        ...
        combined_output_path = os.path.join(output_dir, f"{base_name}_combined_analysis_int8.jpg")
        cv2.imwrite(combined_output_path, combined_viz)
        
        # Create distance matrix visualization
        if len(detections) > 1:
            distance_matrix_path = os.path.join(output_dir, f"{base_name}_distance_matrix_int8.png")
            self._create_distance_matrix(detections, distances, distance_matrix_path)
        
        # Save JSON report
        report = {
            'image': image_path,
            'model_type': 'YOLOv11_TFLite_FLOAT32',  # Using float32 until INT8 is fixed
            'note': 'INT8 model has quantization issues (all class predictions = 0), using FLOAT32',
            'inference_times': {
                'yolo_ms': yolo_time * 1000,
                'depth_ms': depth_time * 1000
            },
        ...
        report_path = os.path.join(output_dir, f"{base_name}_analysis_report_int8.json")
        ...
        print(f"  - Depth inference: {depth_time*1000:.1f}ms")
    ...
    def _create_combined_visualization(self, original: np.ndarray, yolo_result: np.ndarray,
        ...
        cv2.putText(title_bar, "INTEGRATED DETECTION + DEPTH ANALYSIS (TFLite FLOAT32)", 
        ...
        # Draw statistics
        y_offset = 30
        cv2.putText(stats_panel, f"Total Detections: {len(detections)} | Model: TFLite FLOAT32", 
    ...
    def _create_distance_matrix(self, detections: List[Dict], distances: List[Dict], output_path: str):
        ...
        ax1.set_title('Euclidean Distance (pixels) - TFLite FLOAT32', fontsize=12, fontweight='bold')
        ...
        ax2.set_title('Depth Difference - TFLite FLOAT32', fontsize=12, fontweight='bold')
...
def main():
    """
    Main execution function demonstrating pipeline usage with INT8 model.
    
    NOTE: The current INT8 model (best_int8.tflite) appears to have a quantization issue
    where all class prediction outputs are at the zero-point (producing all zeros after
    dequantization). This results in no detections.
    
    ISSUE DIAGNOSIS:
    - Bbox channels (0-3): Properly quantized with varied values
    - Class channels (4-8): All stuck at zero_point (5), dequantize to 0
    - After sigmoid(0) = 0.5, all confidences are below threshold
    
    SOLUTIONS:
    1. Re-export INT8 model with proper quantization calibration
    2. Use float32 model (best_float32.tflite) which works correctly
    3. Use float16 model for a compromise between size and accuracy
    
    For now, this script will use FLOAT32 model as fallback.
    """
    print("\n" + "="*80)
    print("INTEGRATED DETECTION + DEPTH PIPELINE - TFLite Version")
    print("⚠️  NOTE: INT8 model has quantization issues, using FLOAT32 instead")
    print("="*80 + "\n")
    
    # Configuration - Use FLOAT32 model instead (INT8 appears to have quantization issues)
    ...
    OUTPUT_DIR = '/home/harshyy/Desktop/pipeline_output_int8'
    
    # Check if files exist
    if not os.path.exists(YOLO_MODEL):
        print(f"❌ YOLO INT8 model not found: {YOLO_MODEL}")
        print("Please ensure the INT8 model is exported or update the path.")
        return
    
    if not os.path.exists(YOLO_CLASSES):
        print(f"❌ Classes file not found: {YOLO_CLASSES}")
        return
    ...
    pipeline = IntegratedPipelineTFLiteINT8(
        ...
        yolo_conf_threshold=0.51,  # Lowered to match model output (sigmoid already applied)
    ...
    print("="*80)
    print(f"\nModel Type: TFLite FLOAT32 (INT8 has quantization issues)")
